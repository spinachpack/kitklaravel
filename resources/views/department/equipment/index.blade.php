                ...
                <tbody>
                    @forelse($equipment as $item)
                        <tr>
                            <td>
                                @if($item->image)
                                    <img src="{{ asset('uploads/equipment/' . $item->image) }}" class="rounded" style="width: 60px; height: 60px; object-fit: cover;" alt="">
                                @else
                            ...
                            <td>
                                ...
                                <form action="{{ route('department.equipment.destroy', $item->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
...
<div class="modal fade" id="addModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('department.equipment.store') }}" enctype="multipart/form-data">
                @csrf
...
function editEquipment(item) {
    document.getElementById('editForm').action = `/department/equipment/${item.id}`;
    ...
    if (item.image) {
        const imageUrl = '/uploads/equipment/' + item.image;
